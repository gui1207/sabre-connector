...
public class Flight_ {
    ...
    @JsonProperty("Fare")
    public Fare__ getFare() {
        return fare;
    }

    @JsonProperty("Fare")
    public void setFare(Fare__ fare) {
        this.fare = fare;
    }
    ...
    public String toString() {
        return new ToStringBuilder(this).append("originLocation", originLocation).append("destinationLocation", destinationLocation).append("airline", airline).append("sideTrip", sideTrip).append("reservation", reservation).append("mileageDisplay", mileageDisplay).append("bookingDateTime", bookingDateTime).append("fare", fare).append("plusUp", plusUp).append("surcharge", surcharge).append("number", number).append("departureDateTime", departureDateTime).append("arrivalDateTime", arrivalDateTime).append("marriageStatus", marriageStatus).append("type", type).append("flown", flown).append("classOfService", classOfService).toString();
    }

    @java.lang.Override
    public int hashCode() {
        return new HashCodeBuilder().append(surcharge).append(fare).append(classOfService).append(bookingDateTime).append(destinationLocation).append(marriageStatus).append(type).append(sideTrip).append(originLocation).append(flown).append(mileageDisplay).append(number).append(plusUp).append(reservation).append(departureDateTime).append(arrivalDateTime).append(airline).toHashCode();
    }
    ...
    public boolean equals(Object other) {
        ...
        if ((other instanceof Flight_) == false) {
            return false;
        }
        Flight_ rhs = ((Flight_) other);
        return new EqualsBuilder().append(surcharge, rhs.surcharge).append(fare, rhs.fare).append(classOfService, rhs.classOfService).append(bookingDateTime, rhs.bookingDateTime).append(destinationLocation, rhs.destinationLocation).append(marriageStatus, rhs.marriageStatus).append(type, rhs.type).append(sideTrip, rhs.sideTrip).append(originLocation, rhs.originLocation).append(flown, rhs.flown).append(mileageDisplay, rhs.mileageDisplay).append(number, rhs.number).append(plusUp, rhs.plusUp).append(reservation, rhs.reservation).append(departureDateTime, rhs.departureDateTime).append(arrivalDateTime, rhs.arrivalDateTime).append(airline, rhs.airline).isEquals();
    }
